...
const INVENTORY_FALLBACK = [{
  ...
  photo1:"https://drive.google.com/thumbnail?id=1X7gHa-J2931albRMSKmQOvmEJGTDkI8C&sz=w1600",photo2:"https://drive.google.com/thumbnail?id=1SssNhdq7kWD4QBYVhvU9fTHrZp8XWlt9&sz=w1600",photo3:"https://drive.google.com/thumbnail?id=1xTkuL-2kNebvspArwWCj6krRZmFhKyL3&sz=w1600",photo4:"https://drive.google.com/thumbnail?id=1K1AjUhxDf1A-uGrfHtih9p0dj4s4HLqN&sz=w1600"
}];
...
function loadGoogleSheetsJsonp(url){
  return new Promise((resolve,reject)=>{
    const callbackName = `forgepcSheets_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    const script = document.createElement('script');
    const timeout = setTimeout(()=>{ cleanup(); reject(new Error('Délai dépassé pour Google Sheets')); },10000);
    function cleanup(){ clearTimeout(timeout); delete window[callbackName]; script.remove(); }
    window[callbackName] = data=>{ cleanup(); resolve(data); };
    script.onerror = ()=>{ cleanup(); reject(new Error('Impossible de charger Google Sheets')); };
    const separator = url.includes('?') ? '&' : '?';
    script.src = `${url}${separator}tqx=${encodeURIComponent('responseHandler:'+callbackName)}`;
...
function parseGoogleVisualization(data){
  ...
  const headers = table.cols.map(c=>String(c.label||c.id||'').toLowerCase().trim());
  return table.rows.map(r=>{
    const values=(r.c||[]).map(c=>c && c.v !== null && c.v !== undefined ? String(c.v) : '');
    return Object.fromEntries(headers.map((h,i)=>[h,values[i]||'']));
  }).filter(x=>Object.values(x).some(v=>String(v).trim()!=='')).map(normalize);
}
...
async function getInventory(){
  ...
  }catch(e){ console.warn('Lecture CSV Google Sheets échouée, essai JSONP.',e); }

  try{
    const data=await loadGoogleSheetsJsonp(SHEET_CSV_URL);
    const rows=parseGoogleVisualization(data);
    if(rows.length) return rows;
  }catch(e){ console.warn('Lecture Google Sheets impossible; données locales utilisées.',e); }
